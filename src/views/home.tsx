...
You’ll also find your work calendar there. Use it to block out days you’re unavailable so you don’t get booked when you’re busy. When chatting, keep things social and flirty, but don’t talk work in messages. If someone wants a job doing, send them to the Hiring tab so everything stays secure and protected. To get fully verified and start taking jobs, upload the front and back of your ECS or CSCS (or equivalent) plus a quick selfie. Takes less time than using a spirit-level. If you don’t want to appear in the Social tab, you can switch that off in Settings, or control when you show socially. We’re in open beta, so if anything feels dodgy, broken, or you’ve got ideas to improve things, drop me a message anytime. `
        : `Hello ${fallbackName}! Welcome to GayTradies. Head to your profile and upload a photo to get started. Torso shots are fine, just no nudity please. When messaging Tradies, keep things social and flirty. If you need work done, use the Hiring tab to book properly so everything stays secure. Payments are protected and only released once you’ve marked the job as completed. No awkward chasing, no stress. We’re in open beta, so if you spot bugs, have feature ideas, or something doesn’t feel right, drop me a message.`;
  ...
  const handleWinkBackFromChip = useCallback(async (p) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'artifacts', getAppId(), 'public', 'data', 'notifications'), {
        ...
        senderName,
        senderPhoto,
        message: `${senderName} winked back! 😉`,
        createdAt: serverTimestamp(),
        timestamp: serverTimestamp(),
        read: false
      });
            ...
            <div className="flex justify-between items-start">
                ...
                {profile.rate && (
                     <div className="text-right">
                         <span className="block font-mono font-bold text-slate-200">£{profile.rate}/hr</span>
